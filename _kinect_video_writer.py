...
class Kinect():
            ...
            if grid:
                ...
                cv2.rectangle(self.DepthFrame, (self.horizontalPosition, self.verticalPosition),
                              (int(self.horizontalPosition+self._args["boxSize"]*self.distanceRatio),
                               int(self.verticalPosition+self._args["boxSize"]*self.distanceRatio)),
                              (255, 0, 0), 1)
                        
                for x in np.arange(self.horizontalPosition+self._args["gridRes"],
                                   self.horizontalPosition+(self._args["boxSize"] * self.distanceRatio),
                                   self._args["gridRes"]):  # FIXME: long conditions
                    
                    for y in np.arange(self.verticalPosition+self._args["gridRes"],
                                       self.verticalPosition+(self._args["boxSize"]*self.distanceRatio),
                                       self._args["gridRes"]):  # FIXME: long conditions
                        ...
                        cv2.putText(self.DepthFrame, str(int(avg_local_value)), (x-10, y+2),
    ...
    def PlayAndSave(self, display=True, samplingTime=60):
        ...
        if samplingTime != 1:
            ...
            self.TestRGBWriter = cv2.VideoWriter(os.path.join(self.dataDir, 'TestRGB.avi'),
                                                 self._args["fourcc"], 20, (self.wRGB, self.hRGB))
                                                    
            self.TestDepthWriter = cv2.VideoWriter(os.path.join(self.dataDir, 'TestDEPTH.avi'),
                                                   self._args["fourcc"], 20, (self.wDepth, self.hDepth))
        ...
        self.RGBWriter = cv2.VideoWriter(os.path.join(self.dataDir, '{0}_{1}-{2}-{3}_{4}-{5}-{6}.avi'
                                                      .format(*self.RGBString)),
                                         self._args["fourcc"], self.sampledFrameRate, (self.wRGB, self.hRGB))
                                                
        self.DepthWriter = cv2.VideoWriter(os.path.join(self.dataDir, '{0}_{1}-{2}-{3}_{4}-{5}-{6}.avi'
                                                        .format(*self.DEPTH8BitString)),
    ...
    def saveMetaData(self):
        ...
        sheet.write(1, 1, "{0}-{1}-{2}_{3}-{4}-{5}".format(self.time.tm_mday, self.time.tm_mon, self.time.tm_year,
                                                           self.time.tm_hour, self.time.tm_min, self.time.tm_sec))
