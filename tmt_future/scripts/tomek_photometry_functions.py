...
def plotIntermediatePhotometryPlots(data, **kwargs):
    ...
    if kwargs["function"] is None:  # TODO: check
        sink = data
    ...
    for n, bol in enumerate(kwargs["which"]):
        if n == 0 and bol:
            ...
            fig = plt.figure(figsize=(15, 7), dpi=200.)
            ax = plt.subplot(111)
            ax.scatter(movingAverage(standardizedIsosbestic, n=resample),
                       movingAverage(standardizedCalcium, n=resample), alpha=0.5,s=1.)
            ax.plot([min(movingAverage(standardizedIsosbestic, n=resample)),
                     max(movingAverage(standardizedCalcium, n=resample))],
                    line, '--', c="red")
...
def LoadPhotometryData(npy, SRD, start, end, videoLength, rollAvg, optPlots=False, recompute=True, plotargs=None):
    ...
    photometryData = np.load(npy)  # Load the NPY file  # FIXME: example of useless comment
    ...
    source = [finalX, finalRawIsosbestic, finalRawCalcium,
              funcIsosbestic, correctedIsosbestic,
              funcCalcium, correctedCalcium,
              standardizedIsosbestic, standardizedCalcium,
              fittedIsosbestic, dF
              ]
...
def LoadTrackingData(npy, fps, start, end):
    rawCotton = np.load(npy)  # Read the file  # REFACTOR: useless
            ...
            if ind < len(posPeaks) - 1:
                if posPeaks[ind+1] - posPeaks[ind] < PMD:
                    for i in np.arange(0, posPeaks[ind+1] - posPeaks[ind]):
                        detectedPeaksMerged.append(True)  # FIXME: append in loop should be extend
        ...
        if event:
            ...
            if cumulativeEvents >= BL:
                if posPotentialEvent - GD > 0 and posPotentialEvent + GD < len(events):
                    ...
                    else:  # FIXME: useless pass
                        pass
                else:  # FIXME: useless pass
                    pass
        elif event == False :  # FIXME: else
            if newBoutDetected:
...
#        smoothedGraph = lowpassFilter(d, SRD, lowpass[0], lowpass[1])
    ...
    if showStd:
        ...
        ax0.fill_between(np.arange(0, len(meanDataAroundPeaks)),
                         meanDataAroundPeaks, meanDataAroundPeaks + stdDataAroundPeaks,
...
def extract_manual_bouts(manualFile, end_vid, beh) :
    ...
    for s, e in zip(f.iloc[start_pos[0]][1:][f.iloc[start_pos[0]][1:] > 0], f.iloc[end_pos[0]][1:][f.iloc[end_pos[0]][1:] > 0]) :
        for i in np.arange(len(peaks), s, 1):
            peaks.append(False)  # FIXME: extend
        for i in np.arange(s, e, 1):
            peaks.append(True)
            posPeaks.append(i)
