...
class Plot(tracker.TopMouseTracker):
    def __init__(self, **kwargs):
        ...
        self._trackingVideo = os.path.join(self._args["main"]["resultDir"], "Tracking_{0}.{1}"
                                           .format(self._args["main"]["mouse"], self._args["saving"]["savingExtension"]))
    ...
    def TrackingPlot(self, res, plotB=True, plotA=True, cBefore='b', cAfter='r', limit=6):
        ...
        if plotB:
            self.befPlot = ax0.plot([x[0] for x in self.filteredPosBefore], [y[1] for y in self.filteredPosBefore],'-o',
                                    markersize=1, alpha=0.1, solid_capstyle="butt",
                                    color=cBefore, label='Before Initiation')
        if plotA:
            self.aftPlot = ax0.plot([x[0] for x in self.filteredPosAfter], [y[1] for y in self.filteredPosAfter], '-o',
                                    markersize=1, alpha=0.1, solid_capstyle="butt",
                                    color=cAfter, label='After Initiation')
    ...
    def CompleteTrackingPlot(self, **kwargs):
       ...
       # ax2.set_xlim([0,len(Before+After)])
        ...
        if args["line"]:
            self.befPlot = ax5.plot([x[0] for x in self.filteredPosBefore], [y[1] for y in self.filteredPosBefore], '-o',
                                    markersize=1, alpha=args["alpha"], solid_capstyle="butt", color=args["cBefore"],
                                    label='Before Initiation')
            self.aftPlot = ax5.plot([x[0] for x in self.filteredPosAfter], [y[1] for y in self.filteredPosAfter], '-o',
                                    markersize=1, alpha=args["alpha"], solid_capstyle="butt", color=args["cAfter"],
                                    label='After Initiation')
        else:
            self.befPlot = ax5.scatter([x[0] for x in self.filteredPosBefore], [y[1] for y in self.filteredPosBefore],
                                       s=10, alpha=args["alpha"], color=args["cBefore"], label='Before Initiation')
            self.aftPlot = ax5.scatter([x[0] for x in self.filteredPosAfter], [y[1] for y in self.filteredPosAfter],
                                       s=10, alpha=args["alpha"], color=args["cAfter"], label='After Initiation')
        ...
        ax5.text(int(self.ROIWidth)/6, -20, 'Time before : {0}h {1}m {2}s'.format(str(hB), str(mB), str(sB)),
                 fontdict=fontBefore)
        ax5.text(int(self.ROIWidth)/6, -10, 'Dist before : {0}'.format(self.distTraveledBeforeInitiation)+'m',
                 fontdict=fontBefore)
        ax5.text(int(self.ROIWidth)/6+150, -20, 'Time after : {0}h {1}m {2}s'.format(str(hA), str(mA), str(sA)),
                 fontdict=fontAfter)
        ax5.text(int(self.ROIWidth)/6+150, -10, 'Dist after : {0}'.format(self.distTraveledAfterInitiation)+'m',
                 fontdict=fontAfter)
        ...
        if args["save"]:
            # plt.savefig(os.path.join(self._args["main"]["resultDir"], "Complete_Tracking_Mouse_{0}"
            #                          .format(self._mouse)), dpi=500.)
    ...
    def NestingRaster(self, **kwargs):
        ...
        if args["rasterSpread"] is None:  # TODO: check
            rasterSpread = (self._args["plot"]["limit"]+1) / (len(All))
        ...
        if args["minDist"] is not None:  # TODO: check
            for pos,peak in enumerate(peaks):
            ...
            for s, e in zip(tStart, tEnd):
                if s < len(Before+After):
                    if e >= len(Before+After):
                        ...
                        ax2.add_patch(patches.Rectangle((s-tCotton[0], 0), len(Before+After)-s, 10, alpha=0.5,
                                                        fc="blue", ec='None', label="Nesting"))
                        self.totalTimeManual += len(Before+After)-s
                    elif e < len(Before+After):
                        temp += e-s
                        ax2.add_patch(patches.Rectangle((s-tCotton[0], 0), e-s, 10, alpha=0.5,
                                                        fc="blue", ec='None', label="Nesting"))
                        self.totalTimeManual += e-s
            ...
            for s, e in zip(tStartInteract, tEndInteract):
                if s < len(Before+After):
                    if e >= len(Before+After):
                        temp += len(Before+After)-s
                        ax2.add_patch(patches.Rectangle((s-tCotton[0], 0), len(Before+After)-s, 10, alpha=0.5,
                                                        fc="blue", ec='None', label="Cotton Interaction"))
                        self.totalTimeManual += len(Before+After)-s
                    elif e < len(Before+After):
                        temp += e-s
                        ax2.add_patch(patches.Rectangle((s-tCotton[0], 0), e-s, 10, alpha=0.5,
        ...
        if args["save"]:
            plt.savefig(os.path.join(self._args["main"]["resultDir"], "{0}_{1}_{2}.png".
                                     format(self._mouse, args["peakThresh"], args["minDist"])))
    ...
    def make_frame_mpl(self, t):
            ...
            if i < self.threshDisplay:
                try:
                    ...
                    self.trajectoryGraph.set_data(list(zip(*self.livePlotTrajectory[0:i]))[0],
                                                  list(zip(*self.livePlotTrajectory[0:i]))[1])
                except:
                    pass
    ...
    def LiveTrackingPlot(self, res=1, tStartLivePlot=0, tEndLivePlot=5*60, acceleration=1, showHeight=True, showTrace=True):
        ...
        # self.cottonHeightGraph, = self.liveAx0.plot(self.livePlotX[0:1],self.livePlotCotton[0:1],
        #                                             lw=2,c="blue",alpha=0.5)
        self.trajectoryGraph, = self.liveAx2.plot(self.livePlotTrajectory[0][0], self.livePlotTrajectory[1][0], '-o',
                                                  color="blue", alpha=0.1, ms=1.)
        ...
        self.finalClip.write_videofile(os.path.join(self._args["main"]["resultDir"], 'Live_Tracking_{0}.mp4'
                                                    .format(self._args["main"]["mouse"])), fps=10)
