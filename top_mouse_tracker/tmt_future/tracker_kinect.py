...
import top_mouse_tracker.tmt_io as IO
...
class Tracker():
    ...
    def Main(self):
        if self.mode == "light":  # If the regular mode was choosen
            ...
            self.RunSegmentationMouse(self.RGBFrame,
                                      self._args["segmentation"]["threshMinRGB"],
                                      self._args["segmentation"]["threshMaxRGB"])  # Runs the segmentation on the ROI
        ...
        if self.mode == "dark":  # If the regular mode was choosen
            ...
            self.RunSegmentationMouse(self.DEPTHFrame,
                                      self._args["segmentation"]["threshMinDEPTH"],
                                      self._args["segmentation"]["threshMaxDEPTH"])  # Runs the segmentation on the ROI
    ...
    def RunSegmentationMouse(self, frame, threshMin, threshMax):
        ...
        if self.cnts:  # If a contour is found in the binary mask
            ...
            if self._args["segmentation"]["minAreaMask"] < self.area < self._args["segmentation"]["maxAreaMask"]:  # Future computation is done only if the area of the detected object is meaningful
    ...
    def CreateDisplay(self):
        ...
        try:
            ...
            cv2.putText(self.metaDataDisplay, 'Detection : '+str("Successful"), (8, 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)  # Displays the detection status
        except:
            cv2.putText(self.metaDataDisplay,'Detection : '+str("Failed"), (8, 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)  # Displays the detection status
        ...
        cv2.putText(self.metaDataDisplay, 'mask size : '+str(int(self.area))+'px2', (8, 15),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)  # Displays the mask size in real-time
        cv2.putText(self.metaDataDisplay, 'dist : '+str("%.2f" % self._distance)+'cm', (8, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)  # Displays the distance traveled by the object in real-time
        cv2.putText(self.metaDataDisplay, 'speed : '+str("%.2f" % self.realTimeSpeed)+'cm/s', (8, 45),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)  # Displays the mask size in real-time
...
class SavingSegmentation():
    ...
    def __init__(self, **kwargs):
        ...
        self.rawWriter = cv2.VideoWriter(os.path.join(self._args["main"]["resultDir"], '{0}_{1}-{2}-{3}_{4}-{5}-{6}.avi'
                                                      .format(* self.rawString)),
                                         self._args["saving"]["fourcc"],
                                         self._args["saving"]["framerate"], (self._W, self._H,))
        ...
        self.segWriter = cv2.VideoWriter(os.path.join(self._args["main"]["resultDir"], '{0}_{1}-{2}-{3}_{4}-{5}-{6}.avi'
                                                      .format(*self.segString)),\
                                         self._args["saving"]["fourcc"],
                                         self._args["saving"]["framerate"], (self.segW, self.segH))
